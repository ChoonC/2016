...
span <- 7
dat2 <- dat %>%
  inflate(center = unique(dat$X)) %>%   #expands a dataset to include all combinations of X
  mutate(dist = abs(X - center)) %>%    
  filter(dist <= span) %>%              # filter days within 7 days of the current day
  mutate(weight =  1)

dat2 %>% filter(center %in% c(-125, -55)) %>%
  ggplot(aes(X, Y)) +   
  geom_point(aes(alpha = weight)) +       # points within 7 days of -125 and -55
...
Over-training is at its worse when we set a $k=1$. In this case we ill obtain perfect accuracy in the training set because each point is used to predict itself. So perfect accuracy must happen by definition. However, the test set accuracy is slightly better than logistics regression.
...
g2 <- ggplot(true_f) +  
  geom_point(data=train_set, aes(x=X_1, y=X_2, fill=label), cex=5, pch=21) +
  scale_fill_manual(name="", values=c("#00BFC4","#F8766D")) +
  stat_contour(aes(X_1,X_2,z=f_hat), breaks=c(0.5),color="black",lwd=1.5) 
