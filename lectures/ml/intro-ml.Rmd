...
digits <- read_csv(url)
tmp <- lapply( c(1,4,5), function(i){            # row 1, 4, 5
    expand.grid(Row=1:28, Column=1:28) %>%      #grid of 28 by 28
      mutate(id=i, label=digits$label[i],        # label of row 1, 4, 5
             value = unlist(digits[i,-1]))       # unlist to vector
})
...
m<-data.frame(data=cbind(sample(c(1,2),30,replace=TRUE,prob=c(0.5,0.5))), cbind(rnorm(30,0), rnorm(30,2), rnorm(30,4),  rnorm(30,5)))
names(m)<-c("Label", "Col1", "Col2", "Col3", "Col4")
m$Col1[1]   # first row value
m$Col1[3]   # first row value
unlist(m[4,])   #unlist row 4
unlist(m[4,-2])   #unlist row 4, except col 2
tmp1 <- lapply( c(1,4,5), function(i){            # row 1, 4,5
    expand.grid(Row=1:2, Column=1:2) %>%        #grid of 2 by 2
      mutate(id=i, label=m$Label[i],         # label of row 1, 4, 5
             value = unlist(m[i,-1]))        # unlist to vector
})
tmp1 <- Reduce(rbind, tmp1)
...
father.son %>% filter(round(father.son$fheight)==66) %>% summarize(mean(sheight))
```
...
dat <- mutate(dat, glm_f_hat = 
  ...
  mutate(glm_pred=round(glm_f_hat))
...
Note that to distinguish 2s from 7s it might be enough to look at the number of non-white pixels in the upper-left and lower-bottom quadrants:

```{r, echo=FALSE}
tmp <- lapply( c(40,45), function(i){         # row 40 and 45
...
ind1 <- which(row_column$col <= 14 & row_column$row <=14) #top left quadrant
...
# Figure out what the above is doing!!!
r_c <- expand.grid(row=1:4, col=1:4)
i1 <- which(r_c$col<=2 & r_c$row <= 2)
i2 <- which(r_c$col>2 & r_c$row > 2)
i  <- c(i1,i2)
M <- as.matrix(m[,-1])
M <- M > 2
M1 <- rowSums(M[,i1])/rowSums(M)
M2 <- rowSums(M[,i2])/rowSums(M)
dat27 <- mutate(dat27, X_1 = X1, X_2 = X2)
