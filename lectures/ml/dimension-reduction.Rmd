...
We will go ahead and center the data:
...
#dist(X) is distance between every pair X1, X2
#sqrt((0.172--1.1812)^2+(0268--2.511)^2),....
#dist(Z) distance between every X1 and X1
...
Note that by using the transformation above we can summarize the distance between any two pair of twins with just one dimension. We reduced the number of dimensions from two to one with very little loss of information. 
...
plot(pc$sdev)
plot(pc$sdev^2/sum(pc$sdev^2))
summary(pc)$importance[,1:5]    # first 5 component

v <- summary(pc)$importance[3,]  #third row is the cummulative proportion
plot(v)

#Need almost 400 to get close to 99%
```

Plot first 2 PC, taking a sample of 4500
```{r}
data.frame(PC1 = pc$x[,1], PC2 = pc$x[,2],
  ...
  geom_point(cex=3, pch=21)
```
The first PC does pretty good job at separating 0 and 1. A transformation which is very big when it is a 1 and small when it is a 0.

Third and fourth PCA

```{r}
data.frame(PC3 = pc$x[,3], PC4 = pc$x[,4],
           label=factor(train_set$label)) %>%
  sample_n(4500) %>% ggplot(aes(PC3, PC4, fill=label))+
  geom_point(cex=3, pch=21)
```
...
High PC 781-784 - catching ink mistakes.  Not useful can be eliminated as noise.
...
Lets choose first 36 components. explains about 77% of the variance

```{r}

summary(pc)$importance[3,36]
y <- factor(train_set$label)
pc_K <- 36
x <- pc$x[,1:pc_K]
fit <- knn3(x, y, k=5)
X_test <- test_set %>% select(-label) %>% as.matrix()
#apply rotation from the training data pc on the test 
#recall pc <- prcomp(X)et and X is from training data 
pc_test <- predict(pc, X_test)
pred <- predict(fit,
                newdata=data.frame(x = pc_test[,1:pc_K]),
                type="class")
