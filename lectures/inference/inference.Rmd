...
Now, $$E(X^2) = a^2p + b^2(1-p)$$ and $$ E(X)^2 = a^{2}p^{2} + 2abp(1-p)+b^{2}(1-p)^2$$
so $$ Var(X) = a^2p(1-p)+b^2(1-p)(1-(1-p))-2abp(1-p) $$
   $$       = a^2p(1-p)-2abp(1-p)+b^2p(1-p) $$
   $$       = (a-b)^2p(1-p) $$
 $$ std(X) = |a-b|\sqrt{(p(1-p)} $$
...
$$\mbox{SE}(S/25) = \sqrt{p(1-p)} / \sqrt{25}$$
...
sqrt(mean( (error)^2))
```
(variance = mean(error)^-mean(error)^2. The second term is 0)
...
mean(error)             # from Monte Carlo   
sqrt( mean( error^2))   # from Monte Carlo
sigma/sqrt(N)           # theory

hist(error)
```

2. Confirm that the distribution is approximately normal with mean 0 and standard deviation $\sigma / sqrt(N)$
...
4. Now use a Monte Carlo simulation to re run this experiment (keeping `prob_head` fixed) 10000 times. Do we confirm that we cover 95% of the time?
