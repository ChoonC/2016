...
p_hat <- mean(X)
cat("Our estimate of the percent of democrats is", p_hat, "plus or minus", round(sqrt(p_hat*(1-p_hat)/N),5))
```
...
N<- 625
B<- 10^4
error <- replicate(B, {X<- sample(x,N,replace=TRUE)
  mean(X)-p
})
hist(error)
mean(abs(error) > 0.02)
