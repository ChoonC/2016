...
error <- Y- theta
hist(error)
se <- 2*sqrt(theta*(1-theta)/800)   #2 times because theta difference p_hat - (1-phat) = 2p_hat - 1
qqnorm(error/se)
abline(0,1)
```
...
Use ggplot to make a plot of the estimated difference versus day. What do you observe? Just from looking at the plot what would you say is that standard deviation of $\theta$ if we let it change with time?
...
Model says regardless of time, it's theta plus some error. From the plot, it is wrong as it changing with time. We need to adapt the model to account for the time effect. The error is not just the about poll sampling, we need to model for the week effect.
...
# Check for error allowing for week effect. Compute the residual between the diff and average of diff each week.
tab2 <- group_by(tab, week) %>% mutate(resid=diff - mean(diff))
qqnorm(tab2$resid)

# Poll theory. Each poll has different size. We use a rough estimate of 1500
se <- 2*sqrt(0.5*0.5) / sqrt(1500)
qqnorm(tab2$resid/se)
abline(0,1)
...
Now if we restrict ourselves to polls with sample sizes larger than 2,000, then the theory tells us that the standard error for the estimate of the percentage for Obama is $\hat{p}$ (notation from previous section) is 
