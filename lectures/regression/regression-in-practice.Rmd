...
select(fit, nameFirst, nameLast, x, .fitted, y) %>% arrange(desc(x)) %>% tbl_df
```
...
#162 to filter out years where there is a shorten season because of strike, and teams that may not have played 162 games (e.g. weather cancellation)
Teams %>% filter(yearID >= 1961 & G == 162)  %>% summarize(cor(R,BB), cor(R,HR), cor(BB, HR))
```
...
```{r}
dat <- Teams %>% filter(yearID >= 1961 & G == 162 & HR %in%120:130)
dat %>% ggplot(aes(BB, R)) + geom_point() + geom_smooth(method="lm")
dat %>% lm(R ~BB, data=.) %>% tidy(conf.int = TRUE)
...
```{r}
my_data <- Teams %>% filter(yearID >= 1961 & G == 162) %>% mutate(group=cut(HR, quantile(HR, prob=seq(0,1,.1)), include.lowest = TRUE))
my_data %>% ggplot(aes(BB,R)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~group)
