...
Rs <- replicate(B, 
        sample_n( father.son, N, replace=TRUE) %>% summarize(r=cor(fheight, sheight)) %>%
                  .$r
)
dat <- data.frame(R = Rs-r)
ggplot(dat, aes(R)) + geom_histogram()
ggplot(dat) + stat_qq(aes(sample=scale(R)))+geom_abline(intercept=0, slope=1)
dat %>% summarize(sd(R))
...
$$
<<<<<<< HEAD
\mbox{std dev}(Y \mid X=x ) = \sqrt{1-\rho^2} \sigma_y
=======
